...
function repeatString(str, times) {
  return times > 0 && str.length ? str.repeat(times) : '';
}
...
function formatTime(...args) {
  return args.map((el) => el.toString().padStart(2, '0')).join(':');
}
...
function countVowels(str) {
  return str
    .toLowerCase()
    .split('')
    .filter((el) => 'aeiouy'.includes(el)).length;
}
...
function isPalindrome(str) {
  const filtered = str.toLowerCase().replaceAll(/\W/g, '');
...
function encodeToRot13(str) {
  ...
  const result = str.split('').map((char) => {
    ...
    const charIndex =
      index + 13 < alpha.length ? index + 13 : index + 13 - alpha.length;
